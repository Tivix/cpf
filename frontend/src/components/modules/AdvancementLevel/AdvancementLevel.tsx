  ...
  return (
      <div className="flex flex-row">
        <div className="flex flex-col items-center relative">
          <div className="mt-4 mb-2 bg-blue-800 rounded-full w-6 h-6 flex items-center justify-center hover:opacity-50" onClick={toggleOpen}>
          ...
          {isOpen && (
              <>
                ...
                {Object.entries(categories).map(([category, skills]) => (
                    <AccordionCard className="w-full" title={category} expandedByDefault={shouldBeExpandedByDefault}>
                      <AccordionList items={skills.map(({name, description}) => ({
                        key: name,
                        title: name,
        ...
        <Modal open={isModalOpen} onClose={hideModal} title="An example way to pass level">
          {projects.map(({title, overview}) => (
              <div className="text-navy-600 text-base">
                <p>{title}</p>
                <div className="mt-5">
