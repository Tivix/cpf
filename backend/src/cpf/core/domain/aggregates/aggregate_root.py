...
class AggregateRoot(ABC):
    ...
    def produce_event(self, *events: DomainEvent) -> None:
        """produce_event should be used to apply pending events to the aggregate after validating domain invariants."""
        for event in events:
    ...
    @classmethod
    def produces_events(cls, method: Callable[..., DomainEvent | IterableType[DomainEvent] | None]):
        @wraps(method)
